...
public class DialogueUIManager : MonoBehaviour
{
    ...
    {
        // 1. 尝试从缓存获取
        if (DialogueCache.Instance.TryGetDialogueHistory(_username, _npcName, out var history))
        {
        ...
        {
            // 存入缓存
            DialogueCache.Instance.SetDialogueHistory(_username, _npcName, serverHistory);
            // 更新UI
    ...
    private async void OnSendButtonClick()
    {
        ...
        try
        {
            ...
            if (npcReply != null)
            {
                ...
                DialogueCache.Instance.AddDialogueToCache(_username, _npcName, userDialogue, npcReply);
